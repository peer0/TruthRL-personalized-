...
def get_dataset(config):
    ...
    dataset = datasets.load_dataset('RUC-NLPIR/FlashRAG_datasets', config.dataset_name)

    return dataset[config.data_split]
...
if __name__ == '__main__':
    ...
    print(f'length of all_split: {len(all_split)}')
    
    input_query = [sample['question'] for sample in all_split]


    print(f'length of input_query: {len(input_query)}')
    ...
    all_results = [{'question': input_query[i], 'documents': results[i], 'scores': scores[i]} for i in range(len(input_query))]
    ...
    save_name = f'e5_{args.retrieval_topk}_top_{args.dataset_name}_{args.data_split}.json'
    save_path = f'Search-R1/retrieval_corpus/{args.dataset_name}/{save_name}'
