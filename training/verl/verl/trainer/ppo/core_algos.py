...
def compute_grpo_outcome_advantage(
    ...
    enable_dynamic_reward: bool = False,
    ...
    Args:
        ...
        enable_dynamic_reward: `(bool)`
            if True, applies dynamic reward logic: if a group contains at least one +1,
            all non-1 values in that group are changed to -1
    ...
    # print(f'\n\n================token_level_rewards shape: {token_level_rewards.shape}') # (64 * 8, 2048)
    # print(f'\n\n================index shape: {index.shape}') # (64 * 8,)
    # print(f'\n\n================response_mask shape: {response_mask.shape}') # # (64 * 8, 2048)
    scores = token_level_rewards.sum(dim=-1)

    # print(f'\n\n================scores shape: {scores.shape}')# [64 * 8]
    ...
    with torch.no_grad():
        ...
        # print(f'\n\n================id2score length: {len(id2score)}') # [64]
        # print(f'\n\n================id2score keys: {id2score.keys()}')
        # print(f'\n\n================id2score first value length: {len(id2score[list(id2score.keys())[0]])}')
        # print(f'\n\n================id2score first value: {id2score[list(id2score.keys())[0]]}') # [1, 1, 1, 1, 1, 1, 1, 1]
        # print(f'\n\n================enable_dynamic_reward: {enable_dynamic_reward}')

        if enable_dynamic_reward:
            # Branch 1: Dynamic reward logic enabled
            # print(f'\n\n================DYNAMIC REWARD ENABLED - BEFORE MODIFICATION================')
            # for idx in sorted(id2score.keys()):
            #     print(f'Group {idx} original scores: {[s.item() for s in id2score[idx]]}')
            
            # Apply dynamic reward logic: if a group contains at least one +1, change all non-1 values to -1
            for idx in id2score:
                group_scores = torch.stack(id2score[idx])  # Stack tensor values from the group
                # Check if group contains at least one +1 (tensor comparison)
                has_positive_one = torch.any(group_scores == 1.0)
                # if has_positive_one:
                #     print(f'Group {idx} has +1 values, applying dynamic reward logic')
                # Find indices in the original scores tensor that belong to this group
                group_indices = [i for i in range(bsz) if index[i] == idx]
                for i in group_indices:
                    # Change all non-1 values to -1 (tensor comparison and assignment)
                    if scores[i].item() != 1.0:
                        scores[i] = -1.0
                # else:
                #     print(f'Group {idx} has no +1 values, no changes made')
            
            # Update id2score with the modified scores
            id2score = defaultdict(list)
            for i in range(bsz):
                id2score[index[i]].append(scores[i])
            ...
            # print(f'\n\n================AFTER MODIFICATION================')
            # for idx in sorted(id2score.keys()):
            #     print(f'Group {idx} modified scores: {[s.item() for s in id2score[idx]]}')
            
            # Calculate id2mean and id2std with updated scores
            id2mean = {}
            id2std = {}
            for idx in id2score:
            ...
            # print(f'\n\n================DYNAMIC REWARD STATISTICS================')
            # for idx in sorted(id2score.keys()):
            #     print(f'Group {idx} - mean: {id2mean[idx].item():.4f}, std: {id2std[idx].item():.4f}')
        else:
            # Branch 2: Original GRPO logic (no dynamic reward)
            # print(f'\n\n================ORIGINAL GRPO LOGIC (NO DYNAMIC REWARD)================')
            # for idx in sorted(id2score.keys()):
            #     print(f'Group {idx} original scores: {[s.item() for s in id2score[idx]]}')
            ...
            # print(f'\n\n================ORIGINAL GRPO STATISTICS================')
            # for idx in sorted(id2score.keys()):
            #     print(f'Group {idx} - mean: {id2mean[idx].item():.4f}, std: {id2std[idx].item():.4f}')
        
        # Apply normalization (same for both branches)
        # print(f'\n\n================NORMALIZATION================')
        ...
        # print(f'First few normalized scores: {[scores[i].item() for i in range(min(8, bsz))]}')
        # print(f'\n\n================scores shape (before unsqueeze): {scores.shape}')
        scores = scores.unsqueeze(-1) * response_mask
        # print(f'\n\n================scores shape (after unsqueeze): {scores.shape}')
        # exit()
