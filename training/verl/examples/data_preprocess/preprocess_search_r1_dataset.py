    ...
    with tempfile.TemporaryDirectory() as tmp_download_dir:
        for split in ["train", "test"]:
            ...
            try:
                ...
                example = df_processed.iloc[0].to_dict()
                example['reward_model']['ground_truth']['target'] = list(example['reward_model']['ground_truth']['target'])
                
                print(example)
                with open(os.path.join(local_save_dir, f"{split}_example.json"), "w") as f:
                    json.dump(example, f, indent=4)
                    logger.info(f"Saved one {split} example to {split}_example.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--local_dir",
        default="/fsx_0/user/zhepei/projects/verl/data/searchR1_processed_direct",
