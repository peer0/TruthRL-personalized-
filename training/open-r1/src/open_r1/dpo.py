...
class ScriptArguments:
    ...
    # max_prompt_length: Optional[int] = field(default=None, metadata={"help": "the maximum prompt length"})
    ...
    for sample in ds:
        ...
        if type(idx0) == np.ndarray or type(idx0) == list:
            ...
            for i in range(len(idx0)):
                ...
                pos.append(sample["responses"][idx0[i]])
                neg.append(sample["responses"][idx1[i]])
                margin.append((sample["rewards"][idx0[i]] - sample["rewards"][idx1[i]]) * margin_scale)
        else:
            # check if the pos and neg response length and do truncation for prompt if needed
            response_1_len = len(tokenizer.encode(sample["responses"][idx0]))
            response_2_len = len(tokenizer.encode(sample["responses"][idx1]))
            longer_response_len = max(response_1_len, response_2_len)
            if longer_response_len + len(tokenizer.encode(P)) > max_length:
                prompt_ids = tokenizer.encode(P, add_special_tokens=False)

                # print(f'max_prompt_length: {max_prompt_length}, max_length: {max_length}, longer_response_len: {longer_response_len}')

                new_prompt_length = min(max_prompt_length, max_length - longer_response_len)
                new_prompt_ids = prompt_ids[:new_prompt_length - 9] + prompt_ids[-9:]
                P = tokenizer.decode(new_prompt_ids)
            ...
            if sample["rewards"][idx0] > sample["rewards"][idx1]:
                ...
                pos.append(sample["responses"][idx0])
                neg.append(sample["responses"][idx1])
                ...
                pos.append(sample["responses"][idx1])
                neg.append(sample["responses"][idx0])
                margin.append((-sample["rewards"][idx0] + sample["rewards"][idx1]) * margin_scale)
    
    # first compare neg and pos response length and get the longer one: response_len_longer
    # if prompt_len + response_len_longer > max_length, then truncate the prompt to max_length - response_len_longer
...
if __name__ == "__main__":
    ...
    # print a random sample from the train_dataset
    print(f'>>>> Traininig Sample:\n\n{train_dataset[0]}')
